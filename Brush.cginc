...
float CalcFeather(float clipinfo, float texoffset)
{
    float feather = (0.5-texoffset)*2.0*_BrushFeatherSig;
    float completion = clipinfo+feather;
    completion = (completion-feather > (_ClipStart) && completion < _ClipEnd) ? 1 : -1;
    return completion;
}

float CalcFeather(float clipinfo, float texoffset, out float preclampcompletion)
{
    float feather = (0.5-texoffset)*2.0*_BrushFeatherSig;
    float completion = clipinfo+feather;
    preclampcompletion = _ClipEnd - completion;
    completion = (completion-feather > (_ClipStart) && completion < _ClipEnd) ? 1 : -1;
    return completion;
